...
def read_char_in_plate(model, img):
    results = model(img)
    bbox_list = results[0].boxes.xyxy.tolist()
    class_names = []
    for i in range(len(bbox_list)):
        cls = int(results[0].boxes.cls[i].item())
        class_names.append(results[0].names[cls])

    if len(bbox_list) < 7 or len(bbox_list) > 10:
        return "unknown"
    ...
    for i, bb in enumerate(bbox_list):
        ...
        y_c = (bb[1] + bb[3]) / 2
        y_sum += y_c
        center_list.append([x_c, y_c, class_names[i]])
    ...
    y_mean = int(int(y_sum) / len(bbox_list))
